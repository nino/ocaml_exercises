...
let find_diffs s1 s2 =
  ...
  List.find_mapi both ~f:(fun i (c1, c2) ->
      if Char.(c1 = c2) then None
      else Some (Printf.sprintf "Diff at index %d: %c <> %c" i c1 c2))

let diff_message s1 s2 =
  match find_diffs s1 s2 with
  | None ->
      if String.length s1 = String.length s2 then ""
      else "Lengths of expected and actual differ but have a common prefix."
  | Some m -> m

let ae exp got _test_ctxt =
  if String.(exp = got) then ()
...
let run_test () =
  run_test_tt_main
    ("beer song tests" >::: List.concat [ verse_tests; lyrics_tests ])
